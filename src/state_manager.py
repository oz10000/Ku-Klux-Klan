...
class StateManager:
    ...
    def save_position(self, position: dict):
        """Guarda una única posición (singular)."""
        self._append("positions.json", position)

    def save_positions(self, positions: list):
        """
        Guarda una lista completa de posiciones (plural).
        Reemplaza el archivo positions.json con la lista actual.
        """
        with open(os.path.join(self.base_dir, "positions.json"), 'w') as f:
            json.dump([p.to_dict() if hasattr(p, 'to_dict') else p for p in positions],
    ...
    def load_all(self):
        data = {}
        files = ['trades', 'positions', 'signals', 'decisions',
                 'trailing_events', 'repairs', 'errors', 'orders']
        for key in files:
