# risk_manager.py
# Krishna Omega Ultra V9.1.1 — Gestión de riesgo con sizing corregido
...
class RiskManager:
    ...
    def check_kill(self) -> bool:
        ...
        if self.current < 20.0:
            ...
            threshold = 40.0 - ratio * (40.0 - KILL_SWITCH_BASE_DD_PCT)
        else:
    ...
    # 🔧 CORREGIDO: calculate_size ahora asegura tamaño > minSz
    def calculate_size(self, entry_price: float, symbol: str, exchange,
        ...
        # 🔥 Si qty < min_sz, forzar min_sz (con factor reducido)
        if qty < min_sz:
            # Ajustar factor para alcanzar min_sz
            needed_factor = (min_sz * entry_price) / (self.current * LEVERAGE)
            needed_factor = min(1.0, needed_factor / factor)  # reducir factor si es necesario
            qty = min_sz
            # Verificar que el capital sea suficiente
            required_margin = (qty * entry_price) / LEVERAGE
            if required_margin > self.current:
                return 0.0
        else:
            # Ajustar por lot_sz
