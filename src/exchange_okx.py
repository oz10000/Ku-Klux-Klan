...
class OKXClient:
    ...
    def set_leverage(self, symbol, leverage, pos_side):
        ...
        body = {"instId": inst_id, "lever": str(leverage), "mgnMode": "isolated", "posSide": pos_side}
        ...
        if resp.get("code") == "0":
            ...
            if ord_id:
                ...
                if detail and detail.get("state") == "filled":
                    ...
                    if avg_px > 0:
                        logger.info(f"Orden ejecutada a precio promedio {avg_px}")
                        return resp, avg_px
                    else:
                        logger.warning("Orden ejecutada pero avgPx no disponible")
                else:
                    logger.warning("Orden no ejecutada completamente aún")
        return resp, None
    ...
    def close_position(self, symbol, pos_id=None, pos_side=None, size=None, mode="swap"):
        if mode == "swap":
            if not pos_id or not pos_side:
                ...
                return {"code": "-1"}
            body = {"instId": self._swap_id(symbol), "posId": pos_id, "mgnMode": "isolated", "posSide": pos_side}
